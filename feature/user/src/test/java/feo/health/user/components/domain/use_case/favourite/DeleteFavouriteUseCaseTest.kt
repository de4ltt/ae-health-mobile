...
class DeleteFavouriteUseCaseTest {
    ...
    private val deleteFavouriteUseCase = DeleteFavouriteUseCase(favouritesRepository)

    @Test
    fun `invoke should call deleteFavourite on repository`() = runTest {
        // Arrange
        ...
        coEvery { favouritesRepository.deleteFavourite(catalogItemDomain) } just runs

        // Act
        deleteFavouriteUseCase(catalogItemDomain)

        // Assert
        coVerify(exactly = 1) { favouritesRepository.deleteFavourite(catalogItemDomain) }
    }
}
